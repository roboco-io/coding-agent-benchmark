...
| M-14 | Claude Code를 Upstage Anthropic 호환 엔드포인트로 solar-pro4에 직결하면(thinking 기본값) 격리·무교란 랄프 루프로 RealWorld 백엔드를 상한 30 iter 안에 무개입 완주할 수 있다 (n=3, 완주율 판정·과금 배제. 선행 게이트: 2026-08-19 `/v1/messages` 복구·스모크 200 확인) | 검증 (3/3 완주·재검증 각 2회 일치, 신고 iter 3→2→1. 단 pro4-1은 게이트 오검(@hurl/cli shim이 Hurl 가림, 하네스 귀책)으로 루프 연장 — 소급 재채점으로 유효 iter 3·4:18:23 확정. 전 run 캐시 0, 유효 시간 119–258분로 직결 최장 프로파일) | [EXP-020](../experiments/020-solar-pro4-direct/report.md) |
| M-15 | Codex CLI(`codex exec`) 하네스에서 gpt-6-astra(effort medium)는 격리·무교란 랄프 루프로 RealWorld 백엔드(Hurl 13/13·154/154)를 상한 30 iter 안에 무개입 완주할 수 있다 (n=3, 완주율 판정·과금 배제. GPT-6 계열은 단일 모델 — Anthropic 호환 엔드포인트 부재로 Codex 하네스가 유일 정합 경로) | 검증 (3/3 iter 1 완주 — 게이트 + 재검증 각 2회 일치, 세션 7분대·커밋 3–4회·개입 0. measure v4+hurl 절대 경로로 오검 0건. 시간은 sol 대비 +23–32%로 "1.9x 고속" 미재현 — 속도 서사는 판단 유보) | [EXP-021](../experiments/021-gpt6-astra-codex/report.md) |
...
| M-22 | pi coding agent(`pi -p` v0.87.1)를 제공자 OpenAI 호환 엔드포인트에 직결해 `kimi-k3`·`qwen3.8-max`·`deepseek-flash`(thinking pi 기본값)를 돌리면 격리·무개입 랄프 루프로 RealWorld 백엔드(Hurl 13/13·154/154)를 상한 30 iter·4h 안에 완주할 수 있다 (EN 각 n=3, 완주율 판정·과금 배제) | 검증 (kimi·qwen·flash 각 3/3 완주 — 게이트+재검증 각 2회 일치, 응답 모델 필드 전수 일치. 8 run iter 1, qwen-en-1은 포트 3000 외부 점유 교란으로 iter 5 기록·iter 1 코드 재채점 통과. 세션 flash 1.9–4.1분(D-3 재실행 반영)·kimi 9.5–9.9분·qwen 15.6–29.0분) | [EXP-029](../experiments/029-pi-openweight/README.md) |
| M-23 | pi coding agent(`pi -p` v0.87.1)로 `anthropic/claude-opus-5-5`(Anthropic API 키 직결)·`openai-codex/gpt-6-sol`(ChatGPT OAuth)을 돌리면(thinking pi 기본값) 격리·무개입 랄프 루프로 RealWorld 백엔드(Hurl 13/13·154/154)를 상한 30 iter·4h 안에 완주할 수 있다 (EN 각 n=3, 완주율 판정·과금 배제) | 검증 (opus 3/3·sol 3/3 전 run iter 1 완주 — 게이트+재검증 각 2회 일치, 응답 모델 필드 전수 일치, thinking 전 run `medium`. 세션 opus 3.1–4.9분·sol 4.2–5.6분으로 네이티브 기준선 EXP-026·EXP-027과 범위가 겹침, output은 pi 쪽이 낮음(opus 16.6–19.8K, sol 7.0–8.6K). 에이전트·API 경로가 함께 달라 조합 전체의 차이로만 해석) | [EXP-030](../experiments/030-pi-frontier/report.md) |
